...
with open ("../Data/short_e.txt", "r") as myfile:
    shorte=myfile.readlines()
...
for copy in range(copies):
    ...
    # Add page number
    i = template.index("CVC-N PPP \\hfill Student Copy\n")
    template[i] = template[i].replace("PPP", str(copies-copy), 1)
    j = template.index("CVC-N PPP \\hfill Teacher Copy\n")
    template[j] = template[j].replace("PPP", str(copies-copy), 1)

    # Choose random words
    words = []
    words += random.sample(shorta, 4)
    words += random.sample(shorte, 4)
    words += random.sample(shorti, 4)
    words += random.sample(shorto, 4)
    words += random.sample(shortu, 4)

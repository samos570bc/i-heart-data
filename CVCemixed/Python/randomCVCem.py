...
for copy in range(copies):
    ...
    # Add page number
    i = template.index("CVCe-M PPP \\hfill Student Copy\n")
    template[i] = template[i].replace("PPP", str(copies-copy), 1)
    j = template.index("CVCe-M PPP \\hfill Teacher Copy\n")
    template[j] = template[j].replace("PPP", str(copies-copy), 1)

    # Choose random words
    words = []
    words += random.sample(shortaw, 3)
    words += random.sample(shortiw, 3)
    words += random.sample(shortow, 3)
    words += random.sample(shortuw, 1)
    words += random.sample(shortan, 3)
    words += random.sample(shortin, 3)
    words += random.sample(shorton, 3)
    words += random.sample(shortun, 1)
